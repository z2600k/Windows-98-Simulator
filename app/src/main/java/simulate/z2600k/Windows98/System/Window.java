...
public class Window extends ElementContainer {
    ...
    public final void onDraw(Canvas canvas, int x, int y) {
        if(active || backupBitmap == null){  // 如果我们的内存不足，可能无法创建backupBitmap
            onNewDraw(canvas, x, y);
            drawTopLayer(canvas, x, y);
        }
        else{
            if(shouldRedraw){  // 第一个非活动帧或窗口不活动但已更新
    ...
    public void onNewDraw(Canvas canvas, int x, int y) {  // если не active, то не надо перерисовываться, и этот метод не вызывается
        ...
        if(active) {
            p1.setShader(new LinearGradient(x + border_width, y + border_width, x + width - border_width, y + border_width + 18, Color.parseColor("#000080"), Color.parseColor("#1084D0"), Shader.TileMode.CLAMP));
            p.setColor(Color.parseColor("#000080"));  // синий
        }
        else {
            p1.setShader(new LinearGradient(x + border_width, y + border_width, x + width - border_width, y + border_width + 18, Color.parseColor("#808080"), Color.parseColor("#B5B5B5"), Shader.TileMode.CLAMP)); // серый
            p.setColor(Color.parseColor("#808080"));
        }
        canvas.drawRect(x + border_width, y + border_width, x + width - border_width, y + border_width + 18, p);
        canvas.drawRect(x + border_width, y + border_width, x + width - border_width, y + border_width + 18, p1);
        if(icon != null)
            canvas.drawBitmap(icon, x + border_width + 2, y + border_width + 1, null);
        if (active)
            p_title.setColor(Color.WHITE);
        else
            p_title.setColor(Color.parseColor("#C0C0C0"));
        canvas.drawText(shortTitle, x + border_width + (icon == null ? 2 : 21), y + border_width + 13, p_title);  // если нет иконки, то заголовок окна сдвинут влево
    ...
    protected void repositionEverything(boolean repositionElements){
        ...
        shortTitle = shortenTextToThreeDots(title, maxWidth, p);
    ...
    private ContextMenu getContextMenu(boolean showOnlyClose){
        ...
        if(!showOnlyClose) {
            ButtonInList restore = new ButtonInList("还原", getBmp(R.drawable.restore_image_inactive),
            ...
            ButtonInList move = new ButtonInList("移动");
            move.closeMenuAfterClick = false;
            move.action = moveRunnable;
            rightMenu.elements.add(move);
            ButtonInList size = new ButtonInList("大小");
            size.closeMenuAfterClick = false;
            size.disabled = true;
            rightMenu.elements.add(size);
            ButtonInList minimize = new ButtonInList("最小化", getBmp(R.drawable.minimize_image_inactive),
            ...
            ButtonInList maximize = new ButtonInList("最大化", getBmp(R.drawable.maximize_image_inactive),
            ...
            ButtonInList close = new ButtonInList("关闭", "Alt+F4", getBmp(R.drawable.close_image_inactive),
