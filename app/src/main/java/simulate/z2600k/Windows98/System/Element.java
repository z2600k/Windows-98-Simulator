...
public abstract class Element {
    ...
    public static Paint p = new Paint(), p_title,p_bold, p_very_bold, p_fixedsys, p_dos, p_dos_mode, p_system,p_game;  // инициализируются в Windows98
    ...
    public static String[] splitTextIntoLines(String text, int widthLimit, Paint p) {  // 太长的话只是越过了正确的边界。
        return splitTextIntoLines(text, widthLimit, -1, p, true, false);
    }

    public static String[] splitTextIntoLines(String text, int widthLimit, int linesLimit, Paint p) {
        return splitTextIntoLines(text, widthLimit, linesLimit, p, true, false);
    }

    public static String[] splitTextIntoLines(String text, int widthLimit, int linesLimit, Paint p, boolean splitWords, boolean saveDelimiters){  // 将文本分解为行
        // splitWords - 将一个太长的单词分成两行或更多行（否则将其留在屏幕边缘）
        // saveDelimiters - 在每行的末尾都有一个分隔符（空格或\n，需要保持文本长度）
        // 将文本拆分为单词、空格和\n，并将其包含在单词的末尾
    ...
    protected void onDraw2(Canvas canvas, int i, int i1) {
    }
