...
import simulate.z2600k.Windows98.Applications.Explorer;
import simulate.z2600k.Windows98.Applications.FileProperties;
import simulate.z2600k.Windows98.Applications.FreeCell;
import simulate.z2600k.Windows98.Applications.InternetExplorer;
import simulate.z2600k.Windows98.Applications.Link;
import simulate.z2600k.Windows98.Applications.MPlayer;
import simulate.z2600k.Windows98.Applications.Minesweeper;
import simulate.z2600k.Windows98.Applications.MsDos;
import simulate.z2600k.Windows98.Applications.MyComputer;
import simulate.z2600k.Windows98.Applications.MyDocuments;
import simulate.z2600k.Windows98.Applications.Notepad;
import simulate.z2600k.Windows98.Applications.PaintBrush;
import simulate.z2600k.Windows98.Applications.RecycleBin;
import simulate.z2600k.Windows98.Applications.Solitaire;
import simulate.z2600k.Windows98.Applications.Spider;
import simulate.z2600k.Windows98.Applications.StartMenu;
import simulate.z2600k.Windows98.Applications.WordPad;
...
public class Windows98 extends ElementContainer {
    ...
    AndroidIcon androidLogo=new AndroidIcon();
    ...
    private void loadWindows(){
        updateScreenSize();
        // loading fonts
        Typeface tf = Typeface.createFromAsset(context.getAssets(),"simsun.ttf");
        synchronized(Element.p) {
            Element.p.setTypeface(tf);
            //Element.p.setTextSize(8);
            Element.p.setTextSize(12);
        ...
        Typeface tf_title = Typeface.createFromAsset(assets,"simsun.ttf");
        Element.p_title = new Paint();
        Element.p_title.setTypeface(tf_title);
        Element.p_title.setTextSize(12);
        Element.p_title.setAntiAlias(false);
        Typeface tf_bold = Typeface.create(Typeface.createFromAsset(assets,"simsun.ttf"),Typeface.BOLD);
        Element.p_bold = new Paint();
        Element.p_bold.setTypeface(tf_bold);
        Element.p_bold.setTextSize(12);
        Element.p_bold.setAntiAlias(false);
        Typeface tf_game = Typeface.createFromAsset(assets,"simsun.ttf");
        Element.p_game = new Paint();
        Element.p_game.setTypeface(tf_game);
        Element.p_game.setTextSize(14);
        Element.p_game.setAntiAlias(false);
        Element.p_game.setColor(Color.parseColor("#FFFFFF"));
        Typeface tf_very_bold = Typeface.create(Typeface.createFromAsset(assets,"simsun.ttf"),Typeface.BOLD);
        ...
        final Link myComputer = new Link(0, 0, "我的电脑", getBmp(R.drawable.computer_explorer_2), false, MyComputer.class);
        ...
        links.elements.add(new Link(0, 3, "Android 应用",getBmp(androidLogo.GetAndroidLogo("Normal")), true, AndroidApps.class));
        links.elements.add(new Link(narrow? 3 : 4, 2, "扫雷", getBmp(R.drawable.winmine_bigicon), true, Minesweeper.class));
        links.elements.add(new Link(narrow? 4 : 5, 2, "蜘蛛纸牌", getBmp(R.drawable.spider_icon), true, Spider.class));
        links.elements.add(new Link(narrow? 3 : 4, 3, "纸牌", getBmp(R.drawable.game_solitaire_0), true, Solitaire.class));
        links.elements.add(new Link(narrow? 4 : 5, 3, "空当接龙", getBmp(R.drawable.game_freecell_2), true, FreeCell.class));
        links.elements.add(new Link(narrow? 6 : 8, 2, "记事本", getBmp(R.drawable.notepad_1), true, Notepad.class));
        links.elements.add(new Link(narrow? 7 : 9, 2, "MS-DOS 方式", getBmp(R.drawable.ms_dos_1), true, MsDos.class));
        links.elements.add(new Link(narrow? 6 : 8, 3, "计算器", getBmp(R.drawable.calculator_0), true, Calculator.class));
        links.elements.add(new Link(narrow? 7 : 9, 3, "写字板", getBmp(R.drawable.write_wordpad_1), true, WordPad.class));
    ...
    private void updateAndroidWallpaper(){
        if(AndroidApps.weAreLauncher()){
            ...
            if(wallpaper == null) {  // голубой экран
                // создаем bitmap 1x1
                androidWallpaper = Bitmap.createBitmap(new int[]{Color.parseColor("#008080")},
    ...
    public static ButtonList newMenu(final Explorer.LinkContainer linkContainer){  // Меню New > (Folder, Shortcut...). Используется ещё в Explorer'е
        ...
        ButtonInList folder = new ButtonInList("文件夹", StartMenu.directoryClosed, false);
        ...
        addNew.elements.add(new ButtonInList("快捷方式",
                getBmp(R.drawable.overlay_shortcut_0), false));
        addNew.elements.add(new Separator());
        ButtonInList textDocument = new ButtonInList("文本文档", getBmp(R.drawable.notepad_file_1), false);
        textDocument.action = parent -> {
            if(linkContainer.getDirectory() == null)
                new Notepad();
            else
                linkContainer.createFile("新建文本文档", ".txt");
        };
        addNew.elements.add(textDocument);
        ButtonInList wordpadDocument = new ButtonInList("写字板文档",
                getBmp(R.drawable.document_1), false);
        wordpadDocument.action = parent -> {
            if(linkContainer.getDirectory() == null)
                new WordPad();
            else
                linkContainer.createFile("新建写字板文档", ".txt");
        };
        addNew.elements.add(wordpadDocument);
        addNew.elements.add(new ButtonInList("BMP 图像",
                StartMenu.paint, false, PaintBrush.class));
        addNew.elements.add(new ButtonInList("声音",
                StartMenu.soundYel, false));
        addNew.elements.add(new ButtonInList("Microsoft 数据链接",
    ...
    public static ContextMenu win98contextMenu(final Explorer.LinkContainer linkContainer){
        ...
        if(linkContainer instanceof DesktopLinks) {
            ...
            activeDesktop.elements.add(new ButtonInList("按 Web 页查看"));
            activeDesktop.elements.add(new ButtonInList("自定义桌面..."));
            activeDesktop.elements.add(new ButtonInList("立即更新"));
            rightMenu.elements.add(new ButtonInList("活动桌面", activeDesktop));
        }
        else{
            ButtonList view = new ButtonList();
            ButtonInList asWebPage = new ButtonInList("按 Web 页");
            ...
            buttonGroup.add(new ButtonInList("大图标"));
            buttonGroup.add(new ButtonInList("小图标"));
            buttonGroup.add(new ButtonInList("列表"));
            buttonGroup.add(new ButtonInList("详细资料"));
            ...
            rightMenu.elements.add(new ButtonInList("查看", view));
            rightMenu.elements.add(new Separator());
            rightMenu.elements.add(new ButtonInList("自定义文件夹..."));
        }
        rightMenu.elements.add(new Separator());

        ButtonList arrangeIcons = new ButtonList();
        arrangeIcons.elements.add(new ButtonInList("按名称"));
        arrangeIcons.elements.add(new ButtonInList("按类型"));
        arrangeIcons.elements.add(new ButtonInList("按大小"));
        arrangeIcons.elements.add(new ButtonInList("按日期"));
        arrangeIcons.elements.add(new Separator());
        arrangeIcons.elements.add(new ButtonInList("自动排列"));
        rightMenu.elements.add(new ButtonInList("排列图标", arrangeIcons));
        rightMenu.elements.add(new ButtonInList("对齐图标"));
        rightMenu.elements.add(new Separator());
        ButtonInList refresh = new ButtonInList("刷新");
        refresh.disabled = true;
        rightMenu.elements.add(refresh);
        ButtonInList paste = new ButtonInList("粘贴", parent -> linkContainer.paste());
        rightMenu.elements.add(paste);
        ButtonInList pasteShortcut = new ButtonInList("粘贴快捷方式");
        pasteShortcut.disabled = true;
        rightMenu.elements.add(pasteShortcut);
        rightMenu.elements.add(new Separator());
        rightMenu.elements.add(new ButtonInList("新建", newMenu(linkContainer)));
        if(!(linkContainer instanceof DesktopLinks))
            rightMenu.elements.add(new Separator());
        ButtonInList properties = new ButtonInList("属性", parent -> {
            if(linkContainer instanceof DesktopLinks)
                new DisplayProperties();
            else if(linkContainer.getDirectory() != null){
                File directory = linkContainer.getDirectory();
                String name = directory.equals(MyDocuments.getFilesDir())? "我的文档" : directory.getName();
                new FileProperties(name, "文件夹", MyDocuments.getFullPath(directory),
