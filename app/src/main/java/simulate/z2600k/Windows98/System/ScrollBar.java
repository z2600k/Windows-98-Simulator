...
public class ScrollBar extends Element {
    ...
    private void calculateThumbCoords(){
        ...
        if(end - start + 1 < 8){  // слишком маленькая кнопка
        ...
        realButtonStart = start + 18;
        realButtonEnd = end + 18;
    }
    ...
    public boolean onMouseOver(int x, int y, boolean touch) {
        ...
        if(!disabled && thumbPress != -1 || (realButtonStart <= length && length <= realButtonEnd)) {  // ползунок
            ...
            if(touch){
                ...
                if(realButtonStart < 18) {
                    realButtonStart = 18;
                    realButtonEnd = realButtonStart + oldWidth;
                }
                else if(realButtonEnd >= (isVertical? height : width) - 18){
                    realButtonEnd = (isVertical? height : width) - 18 - 1;
                    realButtonStart = realButtonEnd - oldWidth;
                }
                startPosition = (double) (realButtonStart - 18) / ((isVertical? height : width) - 36);
                endPosition = (double) (realButtonEnd - 18) / ((isVertical? height : width) - 37);
                scrollable.onScrollbarMoved();
            }
        }
        else if(0 <= length && length < 18) {  // кнопка наверх/влево
        ...
        else if(length < (isVertical? height : width) - 18 && !disabled) {  // 2-й зазор между кнопкой и ползунком
            mouseOverPart = PAGE_DOWN;
            if(touch)
                startAction(pageDown);
        }
        else if(length >= (isVertical? height : width) - 18 && length < (isVertical? height : width)){  // кнопка вниз/вправо
    ...
    private boolean cursorIsNearby() {  // курсор отстоит от нас менее, чем на 34 пикселя
        int x = getCursorX() - getAbsoluteX(), y = getCursorY() - getAbsoluteY();
        return -36 <= x && x < width + 36 && -36 <= y && y < height + 36;
    }
