...
public class MessageBox extends DialogWindow {
    ...
    private static String[] linesTemp = null;  // JAVA 要求第一行要调用 super()
    ...
    public MessageBox(String title, String text, int buttons, Bitmap icon, final MsgResultListener msgResultListener, Window parentWindow){
        ...
        switch (buttons){
            case OK:
                ...
                disableCloseButton();  // 如果没有取消按钮，则不能单击“X”（X 和cancel相等）
                break;
            case OKCANCEL:
                buttonTexts = new String[]{"确定", "取消"};
                break;
            case RETRYCANCEL:
                buttonTexts = new String[]{"重试", "取消"};
                break;
            case YESNO:
                buttonTexts = new String[]{"是", "否"};
                disableCloseButton();
                break;
            case YESNOCANCEL:
                buttonTexts = new String[]{"是", "否", "取消"};
                break;
            default:
