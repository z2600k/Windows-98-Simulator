...
public class MyComputer extends Explorer {
    public MyComputer(){
        ...
        addLink(new Link("3.5 英寸软盘 (A:)", "3.5 寸软盘", getBmp(R.drawable.floppy_drive_3_5), parent -> new MessageBox("我的电脑", "驱动器 A 中的磁盘未被格式化。\n\n想现在格式化吗?",
                MessageBox.YESNO, MessageBox.WARNING, buttonNumber -> {
                    if(buttonNumber == MessageBox.MsgResultListener.YES){  // если пользователь хочет отформатировать дискету в (A:), говорим, что это невозможно
        ...
        addLink(new Link("Android (D:)", "光盘", getBmp(R.drawable.cd_drive_5), parent -> context.checkWriteExternalPermission(new MainActivity.PermissionResultListener() {
            @Override
