...
public class Explorer extends DummyWindow {
    ...
    String defaultHelpText = "选定项目可以查看其说明。";
    ...
    private void initLinkContainer(List<Link> links){
        linkContainer = new LinkContainer(links, new Rect(127, 120, 392, 343),
        ...
        ScrollBar scrollBar = new ScrollBar(linkContainer, new Rect(392, 120, 410, 343),
    ...
    private void initFolderDropdown(File directory){
        ...
        addElement(folderDropdownList, 61, 91);
    }
    ...
    public void onNewDraw(Canvas canvas, int x, int y) {
        ...
        canvas.drawBitmap(folderBmp != null? folderBmp : icon, x + 64, y + 93, null);  // см. folderBmp
        p.setColor(Color.BLACK);
        canvas.drawText(maximized? maximizedFullPath : minimizedFullPath, x + 84, y + 106, p);
        textCanvas.drawBitmap(getBmp(R.drawable.wvleft),0,0,null);
    ...
    public void repositionElements() {
        ...
        if(maximized)
            folderDropdownList.width = Windows98.WIDESCREEN? 791 : 577;
        else
            folderDropdownList.width = 345;
    }
    ...
    public static class LinkContainer extends ScrollElementContainer implements Scrollable {
        ...
        public void createFile(String defaultName, String extension){
            ...
            if(!success) {
                new MessageBox("创建文件出错", "无法创建文件: 文件系统错误。", MessageBox.OK, MessageBox.ERROR, null, parentWindow);
                return;
            }
        ...
        public void paste(){  // вставить файл из буфера обмена
            ...
            if(clipboard == null)
                return;
            if(!clipboard.exists()){
                new MessageBox("复制文件出错", "无法复制文件: 文件系统错误 (1026)。",
            ...
            if(clipboard.isDirectory()){
                ...
                if(src.equals(dst)){
                    new MessageBox("复制文件出错", "无法复制 " + clipboard.getName() + ": 目标文件夹和源文件夹相同。",
                            MessageBox.OK, MessageBox.ERROR, null, parentWindow);
                    return;
                }
                if(dst.startsWith(src + File.separator)){
                    new MessageBox("复制文件出错", "无法复制 " + clipboard.getName() + ": 目标文件夹是源文件夹的子文件夹。",
                            MessageBox.OK, MessageBox.ERROR, null, parentWindow);
                    return;
                }
            }
            ...
            if(newFile.exists()){  // уже есть файл с таким именем
                ...
                while(true){
                    String resultingName;
                    if(number == 1)
                        resultingName = "复件 " + oldName;
                    else
                        resultingName = "复件 (" + number + ") " + oldName;
                    newFile = new File(directory, resultingName);
                    if(newFile.exists()) {
                        number++;
                        continue;
        ...
        private void copyFileError(){
            new MessageBox("复制文件出错", "无法复制文件: 文件系统错误。",
                    MessageBox.OK, MessageBox.ERROR, null, parentWindow);
        }
        ...
        public void initFromDirectory(final File directory, FileProvider fileProvider, final Window parentWindow) {
            ...
            if(parentWindow != null && directory.equals(MyDocuments.getDesktopDirectory())){
                ...
                if(parentWindow instanceof FileDialog)
                    myDocs = new Link("我的文档", "我的文档", getBmp(R.drawable.directory_open_file_mydocs_0),
                            parent -> new FileDialog(MyDocuments.getFilesDir(), (FileDialog) parentWindow));
                else  // MyDocuments
                    myDocs = new Link("我的文档", "我的文档", getBmp(R.drawable.directory_open_file_mydocs_0), MyDocuments.class, parentWindow);
    ...
    private void createTopMenu(boolean isFolder){
        ...
        if(isFolder){
            file.elements.add(new ButtonInList("新建", Windows98.newMenu(linkContainer)));
            file.elements.add(new Separator());
        }
        file.elements.add(new ButtonInList("创建快捷方式"));
        file.elements.add(new ButtonInList("删除"));
        file.elements.add(new ButtonInList("重命名"));
        ...
        edit.elements.add(new ButtonInList("粘贴快捷方式"));
        edit.elements.add(new Separator());
        ...
        edit.elements.add(new ButtonInList("全部选定", "Ctrl+A"));
        ...
        ButtonInList textLabels = new ButtonInList("文本标签");
        ...
        explorerBar.elements.add(new ButtonInList("历史记录", "Ctrl+H"));
        explorerBar.elements.add(new ButtonInList("文件夹"));
        ...
        arrangeIcons.elements.add(new ButtonInList("按驱动器名"));
        arrangeIcons.elements.add(new ButtonInList("按类型"));
        arrangeIcons.elements.add(new ButtonInList("按大小"));
        arrangeIcons.elements.add(new ButtonInList("按可用磁盘空间"));
        arrangeIcons.elements.add(new Separator());
        arrangeIcons.elements.add(new ButtonInList("自动排列"));
        view.elements.add(new ButtonInList("排列图标", arrangeIcons));
        view.elements.add(new ButtonInList("对齐图标"));
        view.elements.add(new Separator());
        view.elements.add(new ButtonInList("刷新"));
        view.elements.add(new ButtonInList("文件夹选项"));
        ButtonList go = new ButtonList();
        go.elements.add(new ButtonInList("后退", "Alt+Left Arrow"));
        go.elements.add(new ButtonInList("前进", "Alt+Right Arrow"));
        for(Element el : go.elements)
            ((ButtonInList) el).disabled = true;
        go.elements.add(new ButtonInList("向上一级", parent -> upOneLevel()));
        go.elements.add(new Separator());
        go.elements.add(new ButtonInList("主页", "Alt+Home"));
        go.elements.add(new ButtonInList("频道指南"));
        go.elements.add(new ButtonInList("搜索 Web"));
        go.elements.add(new Separator());
        go.elements.add(new ButtonInList("邮件"));
        go.elements.add(new ButtonInList("新闻"));
        go.elements.add(new ButtonInList("我的电脑"));
        go.elements.add(new ButtonInList("通讯簿"));
        go.elements.add(new ButtonInList("Internet 呼叫"));
        ...
        ButtonInList about = new ButtonInList("关于 Windows 98");
    ...
    // Python 脚本生成的 Windows 98 文件系统视图

    // 数组的第一个元素是文件夹路径，包括文件夹本身；第二-目录号在上面
    // 文件夹语法：<folder name>/<folder index>
