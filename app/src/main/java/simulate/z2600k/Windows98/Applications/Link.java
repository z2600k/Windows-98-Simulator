...
import simulate.z2600k.Windows98.System.ContextMenu;
...
public class Link extends ElementContainer implements DropdownList.Item {
        ...
        if(path.isDirectory()){
            ...
            action = p -> {
                ...
                if ("txt".equals(extension))
                    ...
                    new MessageBox(fullPath + "\\" + fullFilename, "不支持 Win32 应用程序。",
    ...
    private void createActiveIcon(){
        ...
        activeIcon = createIcon(Color.parseColor("#000080"));
    ...
    private void drawText(String[] lines, Canvas canvas, int x, int y){
        ...
        if(lines != lastTextDrawn || active != lastActive || drawTextCacheBmp == null) {  // надо обновить кеш
            ...
            if (active) {
                ...
                p.setColor(Color.parseColor("#008080"));  // голубой (зелёный) фон рабочего стола, на случай если мы поменяли обои
    ...
    private static ContextMenu getContextMenu(final OnClickRunnable openAction, final Link lnk, boolean addPropertiesMenu){
        ...
        rightMenu.elements.add(new ButtonInList("发送到..."));
        rightMenu.elements.add(new Separator());
        rightMenu.elements.add(new ButtonInList("剪切"));
        rightMenu.elements.add(new ButtonInList("复制", parent -> lnk.copy()));
        rightMenu.elements.add(new Separator());
        rightMenu.elements.add(new ButtonInList("创建快捷方式"));
        ButtonInList delete = new ButtonInList("删除");
        delete.action = parent -> lnk.delete();
        rightMenu.elements.add(delete);
        final ButtonInList rename = new ButtonInList("重命名");
