        ...
        return new FileDialog.OnResultListener() {
            ...
            public void writeToFile(File file) {
                ...
                try (OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(file), Charset.forName("UTF-8"))) {  // так как try-with-resources поддерживается только начиная с API 19
                    ...
                    new MessageBox(getTitle(), "无法保存文件", MessageBox.OK, MessageBox.ERROR, null, BaseNotepad.this);
                }
                if(success) {
                    textChanged = false;
                    openedFile = file;
            ...
            public void openFile(File file) {
                ...
                try(BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), FileCharsetDetector.getFileCharsetName(file)))){
                    ...
                    new MessageBox(getTitle(), "无法打开文件", MessageBox.OK, MessageBox.ERROR, null, BaseNotepad.this);
                }
                if(success) {
                    inputFocus = textBox;
                    textBox.setActive(true);
